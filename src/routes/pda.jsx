...
import PDAModal from '@/components/ui/shared/PDAModal';
import { getRole } from '@/lib/utils';
import { generatePDAPDF } from '@/lib/generatePdaPdf';
...
function PDA() {
  ...
  const handleSave = async (formData) => {
    try {
      if (selectedPDA) {
        await api.put(`pdas/${selectedPDA.id}/`, formData);
        toast.success(t("PDA mis à jour"));
      } else {
        await api.post('pdas/', formData);
        toast.success(t("PDA créé avec succès"));
      }
      setShowModal(false);
      fetchData();
    } catch (error) { 
  ...
  const handleDelete = async (id) => {
    if (!window.confirm(t("Supprimer définitivement ce PDA ?"))) return;
    try {
      await api.delete(`pdas/${id}/`);
      toast.success(t("PDA supprimé"));
      fetchData();
  ...
  return (
    <div className="flex flex-col gap-8 px-10 max-sm:px-4 py-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="font-bold text-2xl text-gray-900">{t("Pro-forma Disbursement Account")}</h1>
          <p className="text-gray-500 font-medium">{t("Estimation des frais portuaires et d'agence")}</p>
        </div>
        <button
          onClick={() => { setSelectedPDA(null); setShowModal(true); }}
        ...
        >
          <Plus className="w-5 h-5" /> {t("Nouveau PDA")}
        </button>
      </div>
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              ...
              <tr><td colSpan="4" className="text-center py-10 text-gray-400 font-medium">{t("Aucun PDA trouvé")}</td></tr>
            ) : filtered.map((pda) => (
              <tr key={pda.id} className="hover:bg-gray-50 transition-colors group">
                <td className="px-6 py-4">
                  <div className="font-bold text-buttonGradientSecondary tracking-tight">{pda.pda_number}</div>
                  <div className="text-sm font-bold text-gray-700 flex items-center gap-1 mt-1">
                    <User className="w-3 h-3 text-gray-400"/> {pda.client_nom}
                  </div>
                </td>
                <td className="px-6 py-4">
                  <div className="font-bold text-gray-800 flex items-center gap-1">
                    <Ship className="w-3.5 h-3.5 text-indigo-400"/> {pda.vessel_name}
                  </div>
                  <div className="text-[11px] font-bold text-gray-400 mt-1 uppercase tracking-tighter">
                    PORT: {pda.port_of_arrival || '---'}
                  </div>
                </td>
                {/* <td className="px-6 py-4 text-right">
                  <div className="font-bold text-lg text-gray-900">{Number(pda.grand_total).toLocaleString()}</div>
                  <div className="text-[10px] text-gray-400 font-bold uppercase">{pda.currency}</div>
                </td> */}
                <td className="px-6 py-4">
