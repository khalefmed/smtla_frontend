    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[95vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <form onSubmit={(e) => { e.preventDefault(); onSave(formData); }} className="overflow-y-auto p-8 space-y-8">
          ...
          {/* SECTION DATES & LOGISTIQUE AVANCÉE */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-1 gap-6 pt-4">
            <div className="space-y-2">
                <label className="text-xs font-bold uppercase text-gray-400 flex items-center gap-1">
                    <MessageSquare className="w-3 h-3"/> Commentaires (Sur le PDF)
                </label>
                <textarea value={formData.commentaire} onChange={(e) => setFormData({...formData, commentaire: e.target.value})} rows="2" placeholder="Conditions de paiement, validité..." className="w-full p-3 bg-gray-50 border rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 text-sm"></textarea>
            </div>
            {/* <div className="space-y-2">
                <label className="text-xs font-bold uppercase text-gray-400 flex items-center gap-1">
                    <ShieldCheck className="w-3 h-3"/> Note Interne (Invisible sur le PDF)
                </label>
                <textarea value={formData.commentaire} onChange={(e) => setFormData({...formData, commentaire: e.target.value})} rows="2" placeholder="Notes pour l'équipe..." className="w-full p-3 bg-indigo-50/30 border border-indigo-100 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 text-sm"></textarea>
            </div> */}
          </div>
        </form>
        ...
        <div className="p-8 border-t bg-gray-50 flex flex-wrap justify-between items-center gap-6">
          ...
          <div className="flex items-center gap-8">
            ...
            <button onClick={(e) => { e.preventDefault(); onSave(formData); }} className="px-10 py-4 bg-buttonGradientSecondary text-white rounded-2xl font-bold hover:shadow-xl hover:scale-[1.02] transition-all active:scale-95">
