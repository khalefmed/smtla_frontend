          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="relative mt-1">
                ...
                <input required type="number" step="0.01" value={formData.weight} onChange={(e) => setFormData({...formData, weight: e.target.value})} className="w-full pl-10 p-3 bg-indigo-50 border border-indigo-100 rounded-xl font-black text-indigo-700 outline-none focus:ring-2 focus:ring-indigo-500" placeholder="Poids total" />
              </div>
            </div>
          </div>
                ...
                {formData.items.filter(i => i.category === cat.id).map((item, idx) => {
                  ...
                  return (
                    <div key={globalIdx} className="flex gap-6 items-center group animate-in slide-in-from-left-2 duration-200">
                      <input placeholder="Désignation" value={item.label} onChange={(e) => updateItem(globalIdx, 'label', e.target.value)} className="flex-1 p-2 border-b border-gray-100 focus:border-indigo-400 outline-none text-sm font-medium" />
                      <div className="flex items-center gap-3">
                         <span className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">GRT/Qty</span>
                         <input type="number" disabled={isLinked} value={item.grt_value} onChange={(e) => updateItem(globalIdx, 'grt_value', e.target.value)} className={`w-24 p-2 rounded-lg text-center text-sm font-bold border border-transparent ${isLinked ? 'bg-indigo-100 text-indigo-700 cursor-not-allowed' : 'bg-gray-50'}`} />
                      </div>
                      <div className="flex items-center gap-3">
                         <span className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Rate</span>
                         <input type="number" step="0.001" value={item.rate} onChange={(e) => updateItem(globalIdx, 'rate', e.target.value)} className="w-24 p-2 bg-gray-50 rounded-lg text-center text-sm font-bold border border-transparent focus:border-indigo-200" />
                      </div>
        ...
        <div className="p-8 border-t bg-gray-50 flex justify-between items-center">
          ...
          <div className="flex gap-4">
            ...
            <button onClick={handleSubmit} className="px-12 py-4 bg-buttonGradientPrimary text-white rounded-2xl font-black shadow-lg hover:scale-[1.02] transition-all flex items-center gap-3 active:scale-95">
              <Calculator className="w-5 h-5"/> {pda ? "Mettre à jour" : "Enregistrer le PDA"}
            </button>
          </div>
        </div>
      </div>
